...
Edited by Angel Rodriguez 2023.'''
...
class movement():
    ...
    def move(self, fwd=0, back=0, ccw=0, cw=0, up=0, down=0, left=0, right=0):
        ...
        if left != 0:
            ...
            left = round(left)
            # Angel - start
            if left < 10: # Do not move since drone needs to move minimum of 20
                pass
            elif left < 20: # Move 20 if left between 10 and 20
                left = 20
                # Use recursion
                self.move(left=left)
            else: # Just move the specified distance
            ...
            right = round(right)
            # Angel - start
            if right < 10: # Do not move since drone needs to move minimum of 20
                pass
            elif right < 20: # Move 20 if right between 10 and 20
                right = 20
                # Use recursion
