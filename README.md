...
The drone's initial location is also initialized at the cartesian point (0,0), the current angle of the drone being 0°, and the altitude after takeoff using one of the drone's onboard attitude sensors.
...
Rotates the drone 90° in a curved path with a given radius and left/right flag to curve to the left or right. The default is a 50cm curve to the left. This function is still in work and may not always work as expected.
```python
# Curve right 100cm
